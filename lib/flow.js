import { getState, setState, clearState, getUser, saveUser } from "./redis";
...
Te ayudo a generar cotizaciones profesionales en menos de 2 minutos desde WhatsApp.

  💰 Créditos: ${credits}
  Vamos a crear tu primera cotización 👇
...
export async function flow(user, raw) {
  ...
  // 🔥 INIT MENU (CRÍTICO)
  if (!state.step) {
    state.step = "cot_cliente";
    await setState(user, state);

    return `👋 Bienvenido

Te ayudo a generar cotizaciones en menos de 2 minutos desde WhatsApp.
...
👤 Nombre del cliente:`;
  ...
  if (!userData.plan) {
    userData.plan = "free";
    userData.credits = userData.credits ?? 3; // 🎁 onboarding
...
4 cantidad`;
  ...
  if (state.step === "edit_item_qty_new") {
    ...
    const list = state.data.items
      .map(
        (i, idx) => `${idx + 1}. ${i.name} (${i.qty} ${i.unit}) - $${i.price}`,
      )
      .join("\n");
...
volver → regresar`;
  ...
  if (state.step === "cot_ubicacion") {
    ...
    state.step = "cot_item";
    await setState(user, state);
    return "🧾 Producto o servicio (ingresa uno por uno):";
  }
  ...
  if (state.step === "cot_qty") {
    ...
    state.step = "cot_unit";
    await setState(user, state);
    return "Unidad (pieza, servicio, m2, ml, etc):";
  }
...
¿Agregar otro? (si/no)`;
  ...
  if (state.step === "cot_confirm") {
    ...
    if (!isPro && (userData.credits || 0) <= 0) {
      return `🚫 Te quedaste sin cotizaciones

Y justo aquí es donde muchos pierden clientes...

Si un trabajo te deja $500 o más:
¿vale la pena perderlo por no poder cotizar?

🚀 Activa PRO y sigue cotizando sin límites:
...
👉 ${process.env.APP_URL}/upgrade`;
    ...
    if (txt === "1") {
      try {
        ...
        console.timeEnd("PDF_GEN");

        const filename = `cotizacion-${Date.now()}.pdf`;
        const url = await uploadFile(pdf, filename);
...
Este es el tipo de cotización que hace que cierres más trabajos.

⚠️ Guarda el PDF (disponible 7 días)

🚀 Tip: los usuarios PRO generan cotizaciones ilimitadas y nunca pierden clientes.
...
${process.env.APP_URL}/upgrade`,
    ...
    if (txt === "4") {
      ...
      const list = state.data.items
        .map(
          (i, idx) =>
            `${idx + 1}. ${i.name} (${i.qty} ${i.unit}) - $${i.price}`,
        )
        .join("\n");
