...
export async function flow(user, raw) {
  ...
  await redis.sadd(`metrics:users:${today}`, user);
  await redis.sadd("metrics:active_users", user);
  const txt = clean(raw);
  // =========================
  //  REFERIDOS
  // =========================
...
Puedes pegar varios 👇`;
  ...
  if (state.step === "edit_item_field") {
    if (txt === "1") {
      ...
      await setState(user, state);
      return "Nuevo nombre de producto/servicio:";
    }
...
O Puedes pegar varios productos en diferentes líneas 👇`;
  }
  ...
  if (state.step === "cot_item") {
    ...
    if (raw.includes("\n") || /^\d+\s+.+\s+\d+(\.\d+)?$/.test(raw)) {
      ...
      if (items.length > 0) {
        ...
        const key = `products:${user}`;

        for (const item of items) {
          await redis.zincrby(key, 1, item.name);
        }
...
Total actual: $${state.data.total}

Puedes pegar más productos o escribe "listo"`;
      }
    }
  ...
  if (state.step === "cot_price") {
    ...
    return `Total actual: $${state.data.total}

Puedes pegar más productos o escribe "listo"`;
  }

  if (state.step === "cot_more") {
    //  permitir seguir agregando productos (bulk)
    if (raw.includes("\n") || /^\d+\s+.+\s+\d+(\.\d+)?$/.test(raw)) {
      const items = parseItemsBulk(raw);
...
Puedes pegar más productos o escribe "listo"`;
    ...
    //  si viene de edición → regresar a menú editar
    if (state.fromEdit) {
...
O confirma 👇`;
    ...
    return "Puedes pegar más productos o escribe 'listo'";
  }
  ...
  if (state.step === "cot_confirm") {
    ...
    console.log("[CONFIRM RECEIVED]", user, raw);
    if (txt === "1") {
...
👉 ${process.env.APP_URL}/api/pay?user=${user}`;
      ...
      if (isLocked) {
        return "⏳ Estamos generando tu cotización, espera un momento...";
      }
...
💼 Ya puedes enviarla a tu cliente o continuar trabajando desde el menú 👇`,
        ...
        //  DISPARAR WORKER INMEDIATO
        await fetch(`${process.env.APP_URL}/api/queue`).catch(() => {});
        setTimeout(() => {
