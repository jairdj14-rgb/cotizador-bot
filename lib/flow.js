...
export async function flow(user, raw) {
  ...
  if (!state.step) {
    ...
    state.data = { items: [] }; // 🔥 IMPORTANTE para no romper items
  ...
  if (!userData.plan) {
    userData.plan = "free";
    userData.credits = userData.credits ?? 2; // 🎁 onboarding
...
🚀 Activa PRO para cotizaciones ilimitadas
...
👉 ${process.env.APP_URL}/upgrade`;
    ...
    if (txt === "1") {
      try {
        ...
        // 🔥 UX POST PDF
        return {
          text: "📄 Cotización lista ⚠️ Este archivo estará disponible por 7 días. Te recomendamos guardarlo.",
          url,
          next: mainMenu(userData),
        };
      } catch (err) {
